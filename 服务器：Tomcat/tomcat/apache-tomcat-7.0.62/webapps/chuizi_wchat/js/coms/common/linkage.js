...
(function(This){
    ...
    This.linkage= {
         ...
         * @param optionVal "全部"option的value，可不传，默认为""
         */
        init: function (id, level,names,values,url,optionVal) {
            ...
            for (var i=0 ; i<level ; i++){
            	model += "<span ><select id='"+names[i]+"' name='"+names[i]+"' level='"+i+"'style='margin-left: 7px;'class='thisSB' ><option value='"+optionVal+"'>全部</option></select></span>"
			}
			model += "</span>" ;
			$('#' + id + '').html(model);
            ...
            $.ajax({
                ...
                success : function(data, status) {
                    ...
                    var level1="<option value='"+optionVal+"'>全部</option>";
            ...
            values = eval(values) ;
            var setFlag = true ;  //立个flag，用来如果出现第一个空值，这个level的option还是应该有的，但是其后边的level就可以为空option了
            if (values.length > 0){
                //从位置2开始循环，因为位置1上边已经初始化过了。应该以names作为循环点，毕竟是以names为基础
                for (var val = 1 ;val < names.length ; val++){
                    //当level就是空的时候，代表以后所有的都是空，所以就不用循环了
                    if (values[0] == '' || values[0] == undefined) {
                        break;
                    }

                    //当这一级是空的时候，以后的级别都应该是空的。下一次循环就可以退出了
                    ...
                    if (setFlag){
                        $.ajax({
                            ...
                            success : function(data, status) {
                                ...
                                var data = eval(data.data) ;
                                var option="<option value='"+optionVal+"'>全部</option>";
                                for (var int = 0; int < data.length; int++) {
